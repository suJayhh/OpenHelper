...
description: >
  ...
  In Gemini CLI, skills are discovered from `.gemini/skills/` and `.agents/skills/`.
  Users can manage skills via `gemini skills list`, `gemini skills install`, etc.
  Activation is consent-based (e.g., `/skills` or prompted approval).
---
...
must implement those patterns directly using Gemini CLI tools.
...
- This agent runs on **Windows PowerShell** (the `Bash` tool executes PowerShell
  on this host). PowerShell expands `$(...)` inside **double-quoted strings**
  and executes embedded subexpressions.
...
- **MANDATORY ESCAPING:** Before placing any dynamic value inside single quotes,
  ...
  $repoName = 'it's-cool'
  ```
- **NEVER** construct a command by concatenating or interpolating variables
...
- **Validation gate:** Before executing ANY command that includes a dynamic
  value, verify the escaped value does NOT contain: `;`, `|`, `&`, `` ` ``,
...
Before any repository name, owner, branch name, or path is used in a command,
it MUST match `^[A-Za-z0-9_.\-/]+$` (or a stricter subset). Reject any value
containing backticks, dollar signs, semicolons, pipes, ampersands, or quote
characters (`"`, `'`, `` ` ``).
...
The static bounding-block approach (`--- BEGIN/END UNTRUSTED DATA ---`) is
**banned**; it gives a false sense of security and is bypassable.
...
- **ALL** `git` commands MUST include the following configuration override
  to prevent hook execution:
  ```
...
6. **Auto-detect default workspace:**
   - Derive `WORKSPACE_ROOT` by walking up from the current directory until a directory containing `.gemini/` or `.agents/` is found.
...
1. Use `WebSearch` to find candidates. Example query:
   `site:github.com pushed:>2026-04-01 language:python stars:<500`
   (adjust the date to ~30 days ago from the current date)
