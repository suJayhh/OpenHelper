...
- Commit analysis limited to 50 commits to prevent resource exhaustion.
- Commit messages truncated to 500 chars and stripped of control characters / bounding-block markers before JSON serialization.
- subprocess.run(..., shell=False) with list arguments only.
- Deterministic JSON output; no raw commit text reaches the LLM context.
...
#     # SECURITY: Disable hooks on every git invocation.
#     result = subprocess.run(
#         ["git", "-c", "core.hooksPath=/dev/null", "-C", repo_path] + list(args),
...
#     """Truncate and strip control characters / bounding-block markers."""
#     msg = msg[:500]
#     allowed = {"\n", "\r", "\t"}
...
#     # SECURITY: Limit to 50 commits to prevent resource exhaustion.
#     output = run_git(repo_path, "log", f"--format={log_format}", "-n", str(limit))
...
#         # SECURITY: Sanitize message before it ever reaches JSON / LLM context.
